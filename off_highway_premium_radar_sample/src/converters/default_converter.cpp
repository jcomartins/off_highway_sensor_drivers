...
#include "off_highway_premium_radar_sample/converters/pcl_radar_point_type.hpp"
...
void DefaultConverter::on_configure()
{
  ...
  publisher_sensor_feedback_ =
    node->create_publisher<off_highway_premium_radar_sample_msgs::msg::SensorFeedback>(
    "~/sensor_feedback", 10);
  publisher_sensor_state_information_ =
    node->create_publisher<off_highway_premium_radar_sample_msgs::msg::SensorStateInformation>(
    "~/sensor_state_information", 10);
  publisher_sensor_broadcast_ =
    node->create_publisher<off_highway_premium_radar_sample_msgs::msg::SensorBroadcast>(
    "~/sensor_broadcast", 10);
  publisher_location_attributes_ =
    node->create_publisher<off_highway_premium_radar_sample_msgs::msg::LocationAttributes>(
    "~/location_attributes", 10);
  // Event based publishers, frequency-based diagnosis not applicable
  publisher_sensor_dtc_information_ =
    node->create_publisher<off_highway_premium_radar_sample_msgs::msg::SensorDtcInformation>(
  ...
  if (send_ego_vehicle_data_) {
    subscriber_ego_vehicle_input_ =
      node->create_subscription<off_highway_premium_radar_sample_msgs::msg::EgoVehicleInput>(
      "~/ego_vehicle_data", 10,
  ...
  measurement_program_service_ =
    node->create_service<off_highway_premium_radar_sample_msgs::srv::MeasurementProgram>(
    "~/set_measurement_program",
    std::bind(
      &DefaultConverter::on_measurement_program, this, std::placeholders::_1,
      std::placeholders::_2));
  sensor_mode_request_service_ =
    node->create_service<off_highway_premium_radar_sample_msgs::srv::SensorModeRequest>(
    "~/request_sensor_mode",
    std::bind(
...
void DefaultConverter::on_ego_vehicle_data(
  const off_highway_premium_radar_sample_msgs::msg::EgoVehicleInput::ConstSharedPtr & msg)
{
...
void DefaultConverter::on_sensor_mode_request(
  const off_highway_premium_radar_sample_msgs::srv::SensorModeRequest::Request::SharedPtr request,
  off_highway_premium_radar_sample_msgs::srv::SensorModeRequest::Response::SharedPtr response)
{
...
void DefaultConverter::on_measurement_program(
  const off_highway_premium_radar_sample_msgs::srv::MeasurementProgram::Request::SharedPtr request,
  off_highway_premium_radar_sample_msgs::srv::MeasurementProgram::Response::SharedPtr response)
{
  response->success = false;
